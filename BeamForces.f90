...
subroutine GetForces (time, beam, pressure_beam, tau_beam, press, force_pressure, u )
  use share_vars
  use FieldExport
  implicit none
  ...
  real (kind=pr), intent (out)	:: tau_beam (0:ns-1)
  real (kind=pr), intent (in)   :: u (0:nx-1, 0:ny-1,1:2)
  real (kind=pr), intent (out)  :: pressure_beam (0:ns-1)
  real (kind=pr), dimension (0:ns-1):: p1,p2,p3,p4,p5,p21,p22, pressure_lin
  real (kind=pr), intent (out)  :: force_pressure (1:2)
  real (kind=pr), intent (in)   :: press (0:nx-1, 0:ny-1)

  !-------------------------------------------------------------------
  ! This is a dummy routine to call various interpolation routines for the beam
  ! so you can compare them
  !----------------------------------------------------------------------

  call GetForcesInterp ( time, beam, pressure_beam, tau_beam,  press, u, force_pressure, 3  )
...
subroutine BeamSurfaceIntegral (time, beam, u, stress_a, stress_b, press)
  use share_vars
  use FieldExport
  use Interpolation
  implicit none
  ...
  real (kind=pr)		:: work1(0:nx-1, 0:ny-1), work2(0:nx-1, 0:ny-1), work3(0:nx-1, 0:ny-1)
  real (kind=pr)		:: work4(0:nx-1, 0:ny-1), penal(0:nx-1, 0:ny-1,1:2)
...
!     P = BicubicInterpolationFD (xb(i), yb(i), press, 0.0,0.0,xl-dx,yl-dy)
  ...
  if (iWalls==0) then
    open (14, file = trim(dir_name)//'/'//trim(simulation_name)//'surface', status = 'unknown', access = 'append') ! Append output data file
...
subroutine GetForcesInterp(time, beam, pressure_beam, tau_beam, press, u, force_pressure, SPL )
  ...
  integer :: i,j,iymin,iymax,ixmin,ixmax,beam_cg_y,beam_cg_x,ix,iy
  real (kind=pr), intent (in)   :: time
  ...
  real (kind=pr), intent (in)   :: press (0:nx-1, 0:ny-1)
  real (kind=pr), intent (in)   :: u (0:nx-1, 0:ny-1,1:2)
  real (kind=pr)                :: xu,yu,xb,yb,x1,y1,x2,y2,delta_smoothing, xu1,xb1,yu1,yb1
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  if (viscous_tensions) call BeamSurfaceIntegral (time, beam, u, stress_a, stress_b, press)
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!   enddo
  ...
  if ((iIteration == 0).and.(viscous_tensions)) then
    ...
    open (14, file = trim(dir_name)//'/'//trim(simulation_name)//'sigma', status = 'unknown', access = 'append') ! Append output data file
    write (14,'(1x, 128(es15.8,1x))') time, (sigma_beam(n), n=0,ns-1,4)
    close (14)
    
    open (14, file = trim(dir_name)//'/'//trim(simulation_name)//'press', status = 'unknown', access = 'append') ! Append output data file
  ...
  open (14, file = trim(dir_name)//'/'//trim(simulation_name)//'int', status = 'unknown', access = 'append') ! Append output data file
  write (14,'(1x, 3(es15.8,1x))') time, fx,fy
  
  ! save energy of the external forces (that should be what's missing in the beam's energy)
  open (14, file = trim(dir_name)//'/'//trim(simulation_name)//'external_power', status = 'unknown', access = 'append') ! Append output data file
