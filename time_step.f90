...
! 	    call SaveGIF(vort, trim(dir_name)//"/vor2/"//trim(namestring)//"_T="//trim(timestring)//".vor", 13, -colorscale, colorscale)	    
	    ...
	    call SaveGIF(abs(maskvx), trim(dir_name)//"/mask/"//trim(namestring)//"_T="//trim(timestring)//".maskvx", 13, 0.0, maxval(abs(maskvx)))
	    call SaveGIF(abs(maskvy), trim(dir_name)//"/mask/"//trim(namestring)//"_T="//trim(timestring)//".maskvy", 13, 0.0, maxval(abs(maskvy)))
