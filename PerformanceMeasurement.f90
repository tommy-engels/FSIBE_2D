...
! 	if (SaveNow) then write(*,*) time_NST
! enddo
! ---------------------------------------
...
module PerformanceMeasurement
  ...
  real function Performance(Start, channel)
    ...
    elseif (Start == "stop") then
      ...
      write (*,*) "Performance encountered a problem (invalid command!)"
      stop
    endif
  ...
  subroutine SavePerformance(time,time_dt, time_nst, time_pressure, time_mask, time_solid, steps_left,dt1)
    ...
    write (10,'( 2(i3,"d ",i2,"h ",i2,"m ",i2,"s "), 2(es11.4,1x)," - ",2(es11.4,1x),4(es11.4,1x,i3,"% "))') days1,hours1,mins1,secs1,days,hours,mins,secs,&
        time,dt1,time_dt,time_sum,&
        time_nst, nint(100.*time_nst/time_sum), time_pressure, nint(100.*time_pressure/time_sum), time_mask, nint(100.*time_mask/time_sum),&
        time_solid, nint(100.*time_solid/time_sum)
    close (10)
!         write (*,'( 2(i3,"d ",i2,"h ",i2,"m ",i2,"s "), 2(es11.4,1x)," - ",2(es11.4,1x),4(es11.4,1x,i3,"% "))') days1,hours1,mins1,secs1,days,hours,mins,secs,&
!         time,dt1,time_dt,time_sum,&
!         time_nst, nint(100.*time_nst/time_sum), time_pressure, nint(100.*time_pressure/time_sum), time_mask, nint(100.*time_mask/time_sum),&
!         time_solid, nint(100.*time_solid/time_sum)
